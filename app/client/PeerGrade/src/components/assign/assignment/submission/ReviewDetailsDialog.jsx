...
const ReviewDetailsDialog = ({ submissionId, open, onClose }) => {
	...
	const getTotalMaxPoints = () => {
		...
		return reviewDetails.criterionGrades.reduce(
	...
	const getPercentageGrade = () => {
		if (!reviewDetails || !reviewDetails.criterionGrades) return 0;
		const totalGrade = reviewDetails.criterionGrades.reduce(
			(total, cg) => total + cg.grade,
			0
		);
		const totalMaxPoints = reviewDetails.criterionGrades.reduce(
			(total, cg) => total + cg.criterion.maxMark,
			0
		);
			...
			<DialogContent className="max-w-7xl h-[90vh] overflow-auto">
				{reviewDetails ? (
						...
						<Card className="my-6">
							...
							<CardContent>
								...
								<div className="mt-4 text-sm text-gray-600">
									...
									<p>
										Peer Review: {reviewDetails.isPeerReview ? "Yes" : "No"}
									</p>
								</div>
							</CardContent>
						</Card>
					...
					<p className="text-center text-gray-600">
						No reviews available for this submission.
					</p>
				)}
			</DialogContent>
		</Dialog>
