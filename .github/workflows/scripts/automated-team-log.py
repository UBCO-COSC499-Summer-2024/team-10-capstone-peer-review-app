...
def fetch_issues(state, since):
    ...
    return [{'title': issue.title, 'state': issue.state, 'assignees': [assignee.login for assignee in issue.assignees], 'labels': [label.name for label in issue.labels]} for issue in issues]
...
def fetch_prs(state, since):
    ...
    return [{'title': pr.title, 'state': pr.state, 'assignees': [assignee.login for assignee in pr.assignees]} for pr in prs if pr.created_at >= since]
