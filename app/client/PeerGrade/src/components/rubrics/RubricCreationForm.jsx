...
const RubricCreationForm = ({
	...
	const validateRubric = () => {
		const isValid =
			...
			rubricData.criteria.every(
				(criterion) =>
					criterion.criteria.trim() !== "" &&
					criterion.ratings.length > 0 &&
					criterion.ratings.every(
						(rating) =>
							rating.text.trim() !== "" &&
							!isNaN(parseFloat(rating.points)) &&
							parseFloat(rating.points) >= 0
					)
			);
	...
	return (
		<Drawer open={isOpen} onOpenChange={setIsOpen}>
			...
			<DrawerContent>
				...
				<div className="p-4 max-h-[80vh] z-[3000] overflow-y-auto">
					<div className="space-y-4">
						...
						<div className="p-4 bg-gray-100 rounded-md">
							...
							<ul className="text-sm space-y-2">
								...
								<li className="flex items-center">
									<Plus className="h-4 w-4 mr-2 text-green-500" /> Add a new
									rating or criterion
								</li>
								<li className="flex items-center">
									<Minus className="h-4 w-4 mr-2 text-destructive" /> Remove a
									rating
								</li>
								<li className="flex items-center">
									<Trash2 className="h-4 w-4 mr-2 text-destructive" /> Remove a
									criterion
								</li>
								<li>
									...
									You must fill out all the fields in order to submit the
									rubric!
								</li>
								<li className="flex items-center">
									<Badge
										variant="outline"
										className="bg-red-200 text-destructive mr-1"
									>
										Negative
									</Badge>{" "}
										...
										<TableCell className="font-medium border-r p-0">
											<div className="flex items-center justify-between p-2">
												{editing === `${criterion.id}-criteria` ? (
													...
													<>
														<span>
															{criterion.criteria || "Enter criteria"}
														</span>
														<Button
															variant="ghost"
															size="icon"
															onClick={() =>
																setEditing(`${criterion.id}-criteria`)
															}
														>
										...
										<TableCell className="border-r p-0">
											<div className="flex">
												{criterion.ratings.map((rating, index) => (
													<div
														key={index}
														className="flex-1 border-r last:border-r-0 relative px-6"
													>
														<div className="flex flex-col py-2">
															<div className="flex items-center justify-between mb-2">
																{editing ===
																`${criterion.id}-rating-points-${index}` ? (
																	<Input
																		type="number"
																		min="0"
																	...
																	<div className="flex items-center">
																		<span
																			className={`text-sm mr-1 ${parseFloat(rating.points) < 0 ? "text-red-500" : ""}`}
																		>
																			{rating.points || "Pts"}
																		</span>
																		<Button
															...
															{editing === `${criterion.id}-rating-${index}` ? (
																...
																<div className="flex items-center">
																	<span className="text-sm mr-1 max-w-[150px] break-words">
																		{rating.text || "Enter description"}
																	</span>
																	<Button
																		variant="ghost"
																		size="icon"
														...
														{index > 0 && (
															<Button
																...
																onClick={() =>
																	removeRating(criterion.id, index)
																}
															>
											...
											<span className="text-sm">
												{Math.max(
													...criterion.ratings.map(
														(rating) => parseFloat(rating.points) || 0
													),
													0
												)}
											</span>
						...
						<Button
							...
							disabled={!isValid || hasNegativePoints}
						>
							Save Rubric
						</Button>
					</div>
