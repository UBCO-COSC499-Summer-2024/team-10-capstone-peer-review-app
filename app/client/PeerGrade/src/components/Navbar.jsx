...
export default function AppNavbar() {
	...
	const reviewAssignments = assignmentsData
		...
		.sort(
			(a, b) => new Date(a.peerReviewDueDate) - new Date(b.peerReviewDueDate)
		)
		.slice(0, 3);

	// Count the number of assignments marked for review
	const reviewsPendingCount = assignmentsData.filter(
		(assignment) => assignment.forReview
	).length;
									...
									<NavigationMenuLink asChild>
										...
										>
											...
											<ListItem
												href="/peer-review"
												title="Go to reviews"
												className="bg-green-100 flex gap-2"
											>
												<svg
