...
const PeerReview = () => {
  ...
  const filteredAssignments = assignmentsData
    ...
    .filter(assignment => 
      (!filterClass || assignment.className === filterClass) &&
      (!searchTerm || 
    ...
    <div className="w-screen p-6">
        ...
        <div className="flex items-center space-x-4 ">
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="bg-white">
              {Array.from(new Set(assignmentsData.map(assignment => assignment.className))).map((className, index) => (
      ...
      <div className={`grid ${view === 'doc_view' ? 'grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4' : 'grid-cols-1'} gap-6`}>
        {filteredAssignments.map(assignment => (
          <Card key={assignment.id} className="w-full p-4 bg-white shadow-md rounded-lg">
            <CardHeader className="mb-4">
              <CardTitle className="text-xl font-bold">{assignment.className}: {assignment.name}</CardTitle>
              <CardDescription className="text-gray-500">{assignment.shortDesc}</CardDescription>
            </CardHeader>
            <CardContent className="flex flex-col space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-gray-700">Due Date: {assignment.dueDate}</span>
                <Link to={`/assignedPR/${assignment.id}`} className="text-blue-500">Open</Link>
              </div>
              <div className="text-gray-500">Peer Review Due: {assignment.peerReviewDueDate}</div>
            </CardContent>
          </Card>
        ))}
      </div>
