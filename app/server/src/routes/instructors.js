...
const instructorsRouter = () => {
	...
	router.get("/my-classes", async (req, res) => {
		...
		const instructorId = "c1c04e72-dd6d-4ef6-a9d3-a0b1c6789574"; // for Postman purposes only
	...
	router.delete("/delete-class/:classID", async (req, res) => {
		...
		try {
			...
			if (classToDelete) {
				...
				res
					.status(200)
					.json({ message: "Class deleted: " + classToDelete.classname });
			} else {
	...
	router.post("/add-student/:classId", async (req, res) => {
		...
		try {
			...
			if (student && classSizeBool) {
				if (
					!existingStudent ||
					(existingStudent && existingStudent.usersInClass.length === 0)
				) {
			...
			res
				.status(500)
				.json({ message: "Failed to add student to class: " + error });
		}
	});
	...
	router.get("/students/:classId", async (req, res) => {
		...
		try {
			...
			res
				.status(500)
				.json({ message: "Failed to retrieve students in class: " + error });
		}
	});
	...
	router.delete("/delete-student/:classId/", async (req, res) => {
		...
		try {
			...
			res
				.status(500)
				.json({ message: "Failed to delete student from class: " + error });
		}
	});
