...
const ForgotPasswordCard = ({ onSwitchToLogin }) => {
  ...
  useEffect(() => {
    if (frgtToken && !password) {
      fetch(`http://localhost:5001/auth/confirm-email?token=${frgtToken}`, {
        method: 'POST',
        headers: {
      ...
      .then(data => {
        ...
        if (data.error && data.error.status === "Server Fail") {
          ...
          console.log(`Received token ${frgtToken} was successfully processed.`);
          setTokenValid(true);
        }
      })
      .catch((error) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (tokenReceived && tokenValid) {
      if (frgtToken && password) {
        ...
        fetch(`http://localhost:5001/auth/reset-password?token=${frgtToken}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
        ...
        .then(data => {
          ...
          if (data.error && data.error.status === "Error") {
            ...
            console.log(`Received token ${frgtToken} was successfully processed.`);
            toast({ title: "Success", description: "Your password was successfully reset! You may now login.", variant: "positive" });
            navigate('/');
            onSwitchToLogin();
          }
        })
      ...
      fetch(`http://localhost:5001/auth/forgot-password`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
      ...
      .then(data => {
        ...
        if (data.error && data.error.status === "Error") {
          ...
          console.log(`Received token ${frgtToken} was successfully processed. Email sent to user.`);
          setEmailSent(true);
          setError('');
        }
      })
  ...
  return (
    <div className='relative space-y-2 w-full'>
      {tokenReceived && 
        <Alert className='absolute top-0 left-0 w-full flex justify-center items-center space-x-2' variant={tokenValid ? 'success' : 'destructive'}>
          <div>
            {tokenValid ? 
          ...
          <div>
            <AlertDescription>
              {tokenValid ? 'The token verification was successful! You can now enter your new password.' : 'The reset password link either expired or was invalid. Please try again later.'}
            </AlertDescription>
          </div>
        </Alert>
      }
      <Card className="w-full max-w-lg p-8 space-y-8 bg-white shadow-md rounded-lg">
        <CardHeader className="text-center">
          ...
          <CardDescription className="text-gray-600">{tokenValid ? "Please enter your new password" : "Please enter your e-mail to reset your password"}</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          {emailSent ?
            <p className="text-green-500 text-sm">An email has been sent to your email address! Please check it for a verification link and reset your password there.</p>
            :
