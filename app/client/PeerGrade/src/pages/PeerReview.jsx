...
const PeerReview = () => {
	...
	return (
		<div className="w-screen p-6">
				...
				<div className="flex items-center space-x-4 ">
						...
						<Button
							className={
								view === "doc_view"
									...
									: "bg-gray-300 text-white-400 shadow-none"
							}
					...
					<DropdownMenu>
						...
						<DropdownMenuContent className="bg-white">
							{Array.from(
								new Set(
									assignmentsData.map((assignment) => assignment.className)
								)
							).map((className, index) => (
								<DropdownMenuItem
									key={index}
									onSelect={() => setFilterClass(className)}
			...
			<div
				className={`grid ${view === "doc_view" ? "grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4" : "grid-cols-1"} gap-6`}
			>
					...
					>
						<CardHeader className="mb-4">
							<CardTitle className="text-xl font-bold">
								{assignment.className}: {assignment.name}
							</CardTitle>
						...
						<CardContent className="flex flex-col space-y-2">
							...
							<div className="text-gray-500">
								Peer Review Due: {assignment.peerReviewDueDate}
							</div>
						</CardContent>
					</Card>
				))}
